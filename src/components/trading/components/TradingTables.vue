  ...
  <div class="trading-card">
    ...
    <div class="trading-card__table-wrapper">
      <transition
        ...
        mode="out-in">
        <open-orders :searchValue="search" v-if='table === 1'/>
        <balance :searchValue="search" v-else-if='table === 2'/>
        <trade-history :searchValue="search" v-else-if='table === 3'/>
        <pricing-alerts :searchValue="search" v-else/>
      </transition>
    </div>
  </div>
</template>
