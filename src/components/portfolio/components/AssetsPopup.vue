  ...
  <div class="assets-popup" @click.self='close'>
    <div class="assets-popup__card" @click='drop = null'>
      ...
      <header class="assets-popup__header">
        <h2 class="assets-popup__title">Assets</h2>
      </header>
      <ul class="assets-popup__list">
        <li class="assets-popup__item" v-for='(asset, index) in assets' :key="asset.exchange+':'+asset.asset+':'+index">
          <div class="assets-popup__row">
            <div class="assets-popup__item-inner">
              <img :src="getExchangeImage( asset.exchange )" alt="image" class="assets-popup__image">
              <span class="assets-popup__name">{{ asset.asset }}</span>
            </div>
            <div class="assets-popup__item-inner">
              <span class="assets-popup__value">{{ asset.totalInUsd }} USDT</span>
              <subtle
                v-if='asset.location === "local"'
                @click.native.stop.prevent='selectDrop(index)'
                class="assets-popup__edit-button">Edit</subtle>
            </div>
          </div>
          <!-- <transition
          ...
          <transition
            v-on:enter="enter"
            v-on:leave="leave">
          <div ref='drop' @click.stop.prevent class="assets-popup__drop" v-if='index === drop'>

            <ul class="assets-popup__drop-list">

                <li class="assets-popup__drop-item">
                  <span class="assets-popup__drop-name">Symbol</span>
                  <input v-model="assets[ index ].asset" required="true" class="assets-popup__drop-value"
                    placeholder="Asset symbol ETH ..." />
                </li>

                <li class="assets-popup__drop-item">
                  <span class="assets-popup__drop-name">Price in USD</span>
                  <input v-model="assets[ index ].priceInUsd" required="true" class="assets-popup__drop-value"
                    placeholder="1 Asset unit, price in USD " />
                </li>

                <li class="assets-popup__drop-item">
                  <span class="assets-popup__drop-name">Balance</span>
                  <input v-model="assets[ index ].amount" required="true" class="assets-popup__drop-value"
                    placeholder="Total of Asset" />
                </li>

                <li class="assets-popup__drop-item">
                  <span class="assets-popup__drop-name">Sescription</span>
                  <input v-model="assets[ index ].description" required="true" class="assets-popup__drop-value"
...
export default {
  ...
  methods: {
    ...
    saveAsset( index ) {
      // this.loading = true;
      this.localAssetUpdate( this.assets[ index ] );
    },
    removeAsset( index ) {
      // this.loading = true;
      this.localAssetRemove( this.assets[ index ] );
    },
    cancel() {
      this.drop = null;
    },
